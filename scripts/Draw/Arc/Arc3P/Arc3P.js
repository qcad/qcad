...
Arc3P.prototype.getArc3P = function() {
    if (isNull(this.point1) || isNull(this.point2)) {
        return undefined;
    }

    if (isNull(this.point3)) {
        return new RLine(this.point1, this.point2);
    }

    if (this.point1.getDistanceTo(this.point2)<RS.PointTolerance ||
            this.point2.getDistanceTo(this.point3)<RS.PointTolerance ||
            this.point3.getDistanceTo(this.point1)<RS.PointTolerance) {

        this.error = qsTr("At least two points are identical");
        return undefined;
    }

    var arc = RArc.createFrom3Points(this.point1, this.point2, this.point3);
    if (!arc.isValid()) {
        return undefined;
    }
