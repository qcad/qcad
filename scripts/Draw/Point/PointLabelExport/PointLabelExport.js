...
PointLabelExport.prototype = new EAction();

PointLabelExport.prototype.beginEvent = function() {
...
PointLabelExport.prototype.getFileName = function() {
    var drawingFileName = this.getDocument().getFileName();
    var fi;
    var fileName;
    var initialPath = "";
    if (drawingFileName.length === 0) {
        var path = RSettings.getStringValue("PointLabelExport/Path", QDir.homePath());
    ...
    if (drawingFileName.length === 0) {
        RSettings.setValue("PointLabelExport/Path", new QFileInfo(fileName).absolutePath());
    }
