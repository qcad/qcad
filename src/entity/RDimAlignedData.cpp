...
bool RDimAlignedData::moveReferencePoint(const RVector& referencePoint,
    ...
    if (referencePoint.getDistanceTo(refDefinitionPoint1) < RS::PointTolerance) {
        definitionPoint = targetPoint;
        autoTextPos = true;
        ret = true;
    }
    else if (referencePoint.getDistanceTo(refDefinitionPoint2) < RS::PointTolerance) {
        definitionPoint = targetPoint;
        autoTextPos = true;
        ret = true;
    }
...
QList<QSharedPointer<RShape> > RDimAlignedData::getShapes(const RBox& queryBox) const {
    Q_UNUSED(queryBox)
