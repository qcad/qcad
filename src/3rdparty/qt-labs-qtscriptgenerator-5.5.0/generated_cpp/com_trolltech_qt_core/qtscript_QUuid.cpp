...
static QScriptValue qtscript_QUuid_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (!_q_self) {
        return context->throwError(QScriptContext::TypeError,
            ...
            .arg(qtscript_QUuid_function_names[_id+5]));
    }
    ...
    return qtscript_QUuid_throw_ambiguity_error_helper(context,
        qtscript_QUuid_function_names[_id+5],
        qtscript_QUuid_function_signatures[_id+5]);
}
...
static QScriptValue qtscript_QUuid_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 2) {
        if ((qMetaTypeId<QUuid>() == context->argument(0).toVariant().userType())
            ...
            QUuid _q_result = QUuid::createUuidV3(_q_arg0, _q_arg1);
            return qScriptValueFromValue(context->engine(), _q_result);
        } else if ((qMetaTypeId<QUuid>() == context->argument(0).toVariant().userType())
    ...
    if (context->argumentCount() == 2) {
        if ((qMetaTypeId<QUuid>() == context->argument(0).toVariant().userType())
            ...
            QUuid _q_arg0 = qscriptvalue_cast<QUuid>(context->argument(0));
            QByteArray _q_arg1 = qscriptvalue_cast<QByteArray>(context->argument(1));
            QUuid _q_result = QUuid::createUuidV5(_q_arg0, _q_arg1);
            return qScriptValueFromValue(context->engine(), _q_result);
        } else if ((qMetaTypeId<QUuid>() == context->argument(0).toVariant().userType())
            && context->argument(1).isString()) {
            QUuid _q_arg0 = qscriptvalue_cast<QUuid>(context->argument(0));
...
QScriptValue qtscript_create_QUuid_class(QScriptEngine *engine)
{
    ...
    for (int i = 0; i < 8; ++i) {
        ...
        proto.setProperty(QString::fromLatin1(qtscript_QUuid_function_names[i+5]),
