...
Q_DECLARE_METATYPE(const QTableWidgetItem*)
...
static QScriptValue qtscript_QTableWidget_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    case 2:
    if (context->argumentCount() == 1) {
        const QTableWidgetItem* _q_arg0 = qscriptvalue_cast<const QTableWidgetItem*>(context->argument(0));
    ...
    case 11:
    if (context->argumentCount() == 0) {
        const QTableWidgetItem* _q_result = const_cast<const QTableWidgetItem*>(_q_self->itemPrototype());
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
