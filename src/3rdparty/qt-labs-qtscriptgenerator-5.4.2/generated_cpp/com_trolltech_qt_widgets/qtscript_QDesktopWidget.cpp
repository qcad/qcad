...
static QScriptValue qtscript_QDesktopWidget_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 0) {
        const QRect _q_result = _q_self->availableGeometry();
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    if (context->argumentCount() == 1) {
        if ((qMetaTypeId<QPoint>() == context->argument(0).toVariant().userType())) {
            QPoint _q_arg0 = qscriptvalue_cast<QPoint>(context->argument(0));
            const QRect _q_result = _q_self->availableGeometry(_q_arg0);
            return qScriptValueFromValue(context->engine(), _q_result);
        } else if (qscriptvalue_cast<const QWidget*>(context->argument(0))) {
    ...
    case 3:
    if (context->argumentCount() == 0) {
        const QRect _q_result = _q_self->screenGeometry();
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    if (context->argumentCount() == 1) {
        if ((qMetaTypeId<QPoint>() == context->argument(0).toVariant().userType())) {
            QPoint _q_arg0 = qscriptvalue_cast<QPoint>(context->argument(0));
            const QRect _q_result = _q_self->screenGeometry(_q_arg0);
            return qScriptValueFromValue(context->engine(), _q_result);
        } else if (qscriptvalue_cast<const QWidget*>(context->argument(0))) {
            const QWidget* _q_arg0 = qscriptvalue_cast<const QWidget*>(context->argument(0));
            const QRect _q_result = _q_self->screenGeometry(_q_arg0);
