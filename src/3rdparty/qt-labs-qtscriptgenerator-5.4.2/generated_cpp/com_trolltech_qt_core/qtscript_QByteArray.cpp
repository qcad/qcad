...
static QScriptValue qtscript_QByteArray_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    case 4:
    if (context->argumentCount() == 0) {
        const char* _q_result = const_cast<const char*>(_q_self->cbegin());
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
    ...
    case 5:
    if (context->argumentCount() == 0) {
        const char* _q_result = const_cast<const char*>(_q_self->cend());
        return qScriptValueFromValue(context->engine(), _q_result);
    }
    break;
    ...
    if (context->argumentCount() == 4) {
        if (qscriptvalue_cast<const char*>(context->argument(0))
            && context->argument(1).isNumber()
            && qscriptvalue_cast<const char*>(context->argument(2))
