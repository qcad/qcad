...
QtScriptShell_QTimeEdit::QtScriptShell_QTimeEdit(QWidget*  parent)
    : QTimeEdit(parent) {}

QtScriptShell_QTimeEdit::QtScriptShell_QTimeEdit(const QTime&  time, QWidget*  parent)
...
{
    QScriptValue _q_function = __qtscript_self.property("dateTimeFromText");
    if (!_q_function.isFunction() || QTSCRIPT_IS_GENERATED_FUNCTION(_q_function)
        || (__qtscript_self.propertyFlags("dateTimeFromText") & QScriptValue::QObjectMember)) {
        return QTimeEdit::dateTimeFromText(text);
    } else {
        QScriptEngine *_q_engine = __qtscript_self.engine();
        return qscriptvalue_cast<QDateTime >(_q_function.call(__qtscript_self,
            QScriptValueList()
...
bool  QtScriptShell_QTimeEdit::eventFilter(QObject*  arg__1, QEvent*  arg__2)
{
...
bool  QtScriptShell_QTimeEdit::hasHeightForWidth() const
{
...
void QtScriptShell_QTimeEdit::initPainter(QPainter*  painter) const
{
...
bool  QtScriptShell_QTimeEdit::nativeEvent(const QByteArray&  eventType, void*  message, long*  result)
{
...
QPaintDevice*  QtScriptShell_QTimeEdit::redirected(QPoint*  offset) const
{
...
QPainter*  QtScriptShell_QTimeEdit::sharedPainter() const
{
...
{
    QScriptValue _q_function = __qtscript_self.property("textFromDateTime");
    if (!_q_function.isFunction() || QTSCRIPT_IS_GENERATED_FUNCTION(_q_function)
        || (__qtscript_self.propertyFlags("textFromDateTime") & QScriptValue::QObjectMember)) {
...
QValidator::State  QtScriptShell_QTimeEdit::validate(QString&  input, int&  pos) const
{
