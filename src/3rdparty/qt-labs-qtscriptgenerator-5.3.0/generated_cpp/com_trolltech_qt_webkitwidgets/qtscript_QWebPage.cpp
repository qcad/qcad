...
static const QWebPage::FindFlag qtscript_QWebPage_FindFlag_values[] = {
    ...
    , QWebPage::FindAtWordBeginningsOnly
...
static const QWebPage::WebAction qtscript_QWebPage_WebAction_values[] = {
    ...
    , QWebPage::DownloadMediaToDisk
    , QWebPage::CopyMediaUrlToClipboard
    , QWebPage::ToggleMediaControls
    , QWebPage::ToggleMediaLoop
    , QWebPage::ToggleMediaPlayPause
    , QWebPage::ToggleMediaMute
...
static QScriptValue qtscript_QWebPage_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 2) {
        QWebPage::Extension _q_arg0 = qscriptvalue_cast<QWebPage::Extension>(context->argument(0));
        const QWebPage::ExtensionOption* _q_arg1 = qscriptvalue_cast<const QWebPage::ExtensionOption*>(context->argument(1));
    ...
    if (context->argumentCount() == 1) {
        ...
        _q_self->setActualVisibleContentRect(_q_arg0);
    ...
    case 17:
    if (context->argumentCount() == 0) {
        bool _q_result = _q_self->shouldInterruptJavaScript();
        return QScriptValue(context->engine(), _q_result);
    }
    break;
    ...
    case 18:
    if (context->argumentCount() == 0) {
        QStringList _q_result = _q_self->supportedContentTypes();
    ...
    if (context->argumentCount() == 1) {
        QString _q_arg0 = context->argument(0).toString();
        bool _q_result = _q_self->supportsContentType(_q_arg0);
        return QScriptValue(context->engine(), _q_result);
    }
    break;
...
static QScriptValue qtscript_QWebPage_static_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 0) {
        ...
        QScriptValue _q_result = context->engine()->newVariant(context->thisObject(), qVariantFromValue((QWebPage*)_q_cpp_result));
