...
static const char * const qtscript_QWebFrame_function_signatures[] = {
    ...
    , ""
    , ""
    , "QPainter arg__1\nQPainter arg__1, RenderLayer layer, QRegion clip\nQPainter arg__1, QRegion clip"
    , ""
...
static QScriptValue qtscript_QWebFrame_prototype_call(QScriptContext *context, QScriptEngine *)
{
    ...
    if (context->argumentCount() == 2) {
        if (qscriptvalue_cast<QPainter*>(context->argument(0))
            && (qMetaTypeId<QWebFrame::RenderLayer>() == context->argument(1).toVariant().userType())) {
            QPainter* _q_arg0 = qscriptvalue_cast<QPainter*>(context->argument(0));
            QWebFrame::RenderLayer _q_arg1 = qscriptvalue_cast<QWebFrame::RenderLayer>(context->argument(1));
            _q_self->render(_q_arg0, _q_arg1);
            return context->engine()->undefinedValue();
        } else if (qscriptvalue_cast<QPainter*>(context->argument(0))
            && (qMetaTypeId<QRegion>() == context->argument(1).toVariant().userType())) {
            QPainter* _q_arg0 = qscriptvalue_cast<QPainter*>(context->argument(0));
