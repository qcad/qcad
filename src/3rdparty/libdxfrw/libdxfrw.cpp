...
//                        writer->writeInt16(72, 4);
        ...
        if (!ent->solid){
            ...
            writer->writeInt16(78, ent->deflinesnum);
            for (int j = 0; j < ent->deflinesnum; j++)
            {
                DRW_HatchDefine* define = ent->defines[j];

                writer->writeDouble(53, define->angle);
                writer->writeDouble(43, define->origin.x);
                writer->writeDouble(44, define->origin.y);
                writer->writeDouble(45, define->delta.x);
                writer->writeDouble(46, define->delta.y);
                writer->writeInt32(79, define->segmentsnum);
                for (int k = 0; k < define->segmentsnum; k++)
                {
                    writer->writeDouble(49, define->segments[k]);
                }
            }
        }
/*        if (ent->deflines > 0){
