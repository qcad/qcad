...
void DRW_MText::parseCode(int code, dxfReader *reader){
    ...
    case 44:
        ...
        break;
    case 71:
        attachmentPoint = reader->getInt32();        
        break;
    case 72:
        drawingDirection = reader->getInt32();
        break;
    case 73:
        lineSpacingStyle = reader->getInt32();
        break;
    default:
        DRW_Text::parseCode(code, reader);
        break;
...
void DRW_Hatch::parseCode(int code, dxfReader *reader){
    ...
    case 52:
        ...
        break;
    case 53:
        define = new DRW_HatchDefine();
        defines.push_back(define);
        define->angle = reader->getDouble();
        break;
    case 43:
        define->origin.x = reader->getDouble();
        break;
    case 44:
        define->origin.y = reader->getDouble();
        break;
    case 45:
        define->delta.x = reader->getDouble();
        break;
    case 46:
        define->delta.y = reader->getDouble();
        break;
    case 79:
        define->segmentsnum = reader->getInt32();
        define->segments.reserve(define->segmentsnum);
        break;
    case 49:
        define->segments.push_back(reader->getDouble());
        break;
    case 73:
...
bool DRW_Hatch::parseDwg(DRW::Version version, dwgBuffer *buf, duint32 bs){
    ...
    if (!solid){
        ...
        for (dint32 i = 0 ; i < deflinesnum; ++i){
            ...
            defines.push_back(define);
            define->angle = buf->getBitDouble();
            define->origin.x = buf->getBitDouble();
            define->origin.y = buf->getBitDouble();
            define->delta.x = buf->getBitDouble();
            define->delta.y = buf->getBitDouble();
            duint16 numDashL = buf->getBitShort();
            DRW_DBG("\ndef line: "); DRW_DBG(define->angle); DRW_DBG(","); DRW_DBG(define->origin.x); DRW_DBG(","); DRW_DBG(define->origin.y);
            DRW_DBG(","); DRW_DBG(define->delta.x); DRW_DBG(","); DRW_DBG(define->delta.y); DRW_DBG(",");
            define->segments.clear();
            for (duint16 i = 0 ; i < numDashL; ++i){
                double lenghtL = buf->getBitDouble();
                define->segments.push_back(lenghtL);
