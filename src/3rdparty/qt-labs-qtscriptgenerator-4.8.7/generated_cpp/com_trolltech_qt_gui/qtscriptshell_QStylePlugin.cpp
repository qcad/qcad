...
QtScriptShell_QStylePlugin::QtScriptShell_QStylePlugin(QObject*  parent)
    : QStylePlugin(parent) {}

QtScriptShell_QStylePlugin::~QtScriptShell_QStylePlugin() {}

void QtScriptShell_QStylePlugin::childEvent(QChildEvent*  arg__1)
{
...
QStyle*  QtScriptShell_QStylePlugin::create(const QString&  key)
{
    QScriptValue _q_function = __qtscript_self.property("create");
    if (!_q_function.isFunction() || QTSCRIPT_IS_GENERATED_FUNCTION(_q_function)
        || (__qtscript_self.propertyFlags("create") & QScriptValue::QObjectMember)) {
        qFatal("QStylePlugin::create() is abstract!");
    } else {
        QScriptEngine *_q_engine = __qtscript_self.engine();
        return qscriptvalue_cast<QStyle* >(_q_function.call(__qtscript_self,
            QScriptValueList()
            << qScriptValueFromValue(_q_engine, key)));
    }
}

void QtScriptShell_QStylePlugin::customEvent(QEvent*  arg__1)
{
...
bool  QtScriptShell_QStylePlugin::event(QEvent*  arg__1)
{
...
bool  QtScriptShell_QStylePlugin::eventFilter(QObject*  arg__1, QEvent*  arg__2)
{
